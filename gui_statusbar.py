...
class ColoredStatusBar(wx.StatusBar):
    def __init__(self, parent, fieldCount=1, fieldColors=()):
        ...
        self.staticTexts = [
                wx.StaticText(parent=self, style=wx.ST_DOTS_END | wx.ST_NO_AUTORESIZE | wx.ALIGN_CENTER)
    ...
    def OnConnectionError(self):
        ...
        self.SetStatusText("Verbindung zum Projektor fehlerhaft", 0)

    def OnConnectionEstablished(self):
        self.SetStatusBackground("green", 0)
        self.SetStatusText("Verbindung zum Projektor OK", 0)

    def OnGeneralStatusMessage(self, message, color=None):
        if not color:
            color = wx.NullColor
        self.SetStatusBackground(color, 1)
